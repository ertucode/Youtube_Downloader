...
class WarnUser:
    ...
    def __post_init__(self):
        ...
        self.width = len(self.text) * 8 if len(self.text) * 8 > 200 else 200
        self.center_window()
        def button_click_event():
            self.root.destroy()

        label = customtkinter.CTkLabel(master=self.root, text=self.text, width = self.width)
        label.grid(row = 0,column = 0 , pady = 5)
        but_width = 120
        button = customtkinter.CTkButton(self.root, text="OK", command=button_click_event, width = but_width)
